# Production image for chips (chips.cpuchip.net).
# One container: builds the Vite client (dist/) and runs the Node server that
# serves the built client AND the same-origin table WebSocket at /ws.
...
# tsx runs the TypeScript server directly (it reuses the shared engine modules).
CMD ["npm", "run", "serve"]
